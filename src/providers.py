...
class MockOfflineProvider(BaseLLMProvider):
    ...
    def generate_with_tools(self, prompt: str, tools_schema: List[Dict[str, Any]], system_prompt: str = "") -> Dict[str, Any]:
        ...
        if "đặt lịch" in prompt_lower and student_match:
            time_first_match = re.search(r"(\d{1,2}:\d{2})[^\d]*(\d{1,2}/\d{1,2}/\d{4})", prompt_lower)
            date_first_match = re.search(r"(\d{1,2}/\d{1,2}/\d{4})[^\d]*(\d{1,2}:\d{2})", prompt_lower)
            if time_first_match:
                datetime_str = f"{time_first_match.group(1)} {time_first_match.group(2)}"
            elif date_first_match:
                datetime_str = f"{date_first_match.group(2)} {date_first_match.group(1)}"
            else:
                datetime_str = "09:00 ngày mai"
            return {
                "type": "tool_call",
